...
class PageController extends AbstractController
{
    ...
    public function leaderboard(TeamRepository $teamRepo, MatchRepository $matchRepo): Response
    {
        ...
        foreach ($teams as $team) {
            ...
            // Récupère tous les matchs terminés où cette équipe a joué
            $matches = $matchRepo->findCompletedByTeam($team);

            foreach ($matches as $match) {
                if ($match->getTeamA() === $team) {
                    if ($match->getScoreTeamA() > $match->getScoreTeamB()) {
                        $wins++;
                    } elseif ($match->getScoreTeamA() < $match->getScoreTeamB()) {
                        $losses++;
                    } else {
                        $draws++;
                    }
                } else { // l'équipe est l'équipe B
                    if ($match->getScoreTeamB() > $match->getScoreTeamA()) {
                        $wins++;
                    } elseif ($match->getScoreTeamB() < $match->getScoreTeamA()) {
                        $losses++;
                    } else {
                        $draws++;
                    }
    ...
    #[Route('/tournaments/{id}/view', name: 'app_tournament_view', requirements: ['id' => '\d+'])]
    public function tournamentView(int $id, \App\Repository\TournamentRepository $tournamentRepository, \App\Repository\MatchRepository $matchRepository): Response
    {
        $tournament = $tournamentRepository->find($id);
        if (!$tournament) {
            throw $this->createNotFoundException('Tournament not found');
        }
        $matches = $matchRepository->findByTournament($id);
    ...
    #[Route('/goto/tournaments', name: 'app_tournaments_dynamic')]
    public function tournamentsDynamic(): Response
    {
        if ($this->isGranted('ROLE_ADMIN')) {
            return $this->redirectToRoute('app_admin_dash_tournament');
        }
