...
function fetchCartSummary() {
    return fetch('/store/cart/summary', { headers: { 'Accept': 'application/json' } })
        .then(r => r.ok ? r.json() : Promise.reject())
        .catch(() => null);
}
function renderCartPopover(data) {
    const content = document.getElementById('cart-popover-content');
    const totalEl = document.getElementById('cart-popover-total');
    if (!content || !totalEl) return;
    if (!data || !Array.isArray(data.items) || data.items.length === 0) {
        content.innerHTML = '<div class="px-4 py-3 text-sm text-gray-400">Your cart is empty</div>';
        totalEl.textContent = '$0.00';
        return;
    }
    const rows = data.items.map(i => `
        <div class="px-4 py-3 flex justify-between items-center border-b border-[#D4AF37]/10">
            <div class="text-sm">
                <p class="font-semibold">${i.name}</p>
                <p class="text-gray-400">x${i.qty} • $${Number(i.unit_price).toFixed(2)}</p>
            </div>
            <div class="text-sm font-bold text-[#D4AF37]">$${Number(i.total).toFixed(2)}</div>
        </div>
    `).join('');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    wrapper.addEventListener('mouseenter', () => {
        ...
        fetchCartSummary().then(renderCartPopover);
        showCartPopover();
    });
    wrapper.addEventListener('mouseleave', () => {
        hoverTimeout = setTimeout(hideCartPopover, 150);
    });
});
